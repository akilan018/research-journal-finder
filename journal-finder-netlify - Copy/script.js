...
function applyFilters() {
    ...
    if (type === "aim" && input.length > 0) {
        ...
        // --- Strict relevance filter (quality over quantity) ---
        // Sort all scored journals descending
        const sorted = scored.sort((a, b) => b.score - a.score);
        const maxScore = sorted.length > 0 ? sorted[0].score : 0;

        // Hard floor: must score at least 40% of the best match AND >= 1.5 absolute
        const relativeThreshold = maxScore * 0.40;
        const absoluteFloor = 1.5;
        const threshold = Math.max(absoluteFloor, relativeThreshold);

        // Score-gap cut-off: stop if a journal's score drops more than 50% vs previous
        const matched = [];
        let prevScore = maxScore;
        for (const s of sorted) {
            if (s.score < threshold) break;           // Below quality floor — stop
            if (prevScore > 0 && s.score < prevScore * 0.50) break; // Sharp drop — stop
            matched.push(s.j);
            prevScore = s.score;
            if (matched.length >= 100) break;         // Hard cap safety net
        }

        filteredResults = matched;
        // Set count label BEFORE renderResults (pass true so it isn't overwritten)
        const bm25Label = matched.length > 0
            ? `Found ${matched.length} journal${matched.length > 1 ? 's' : ''} matching your abstract`
            : "No journals matched your abstract — try different or fewer terms.";
        document.getElementById("resultCount").innerText = bm25Label;
        renderResults(matched, true);
        return;
    }
    // ─────────────────────────────────────────────────────────────────────────
